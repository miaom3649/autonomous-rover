...
class DepthBridgeNode(Node):
    def __init__(self) -> None:
        ...
        self._sub_cmd = self.create_subscription(Twist, "/rover/cmd_vel", self._on_cmd_vel, 10)
    ...
    def _on_range(self, msg: Range) -> None:
        if not np.isfinite(msg.range) or msg.range <= 0:
            return
        with self._lock:
    ...
    def _apply_ultrasonic_correction(self, depth: np.ndarray) -> tuple[np.ndarray, Optional[float]]:
        ...
        off by a ratio that varies with the scene (see README). The ultrasonic
        sensor's forward-facing reading gives one trusted point measurement of
        what's directly ahead; we use it to estimate that ratio for this frame
        and apply it to the whole map. Falls back to the raw (uncorrected) depth
        whenever there's no usable ultrasonic reading. The ultrasonic reading is
        trusted unconditionally otherwise — no plausibility check against the
        AI's own estimate, so a mismatch will scale the whole map by whatever
        ratio results, correct or not.
        ...
        with self._lock:
            ultrasonic_range = self._ultrasonic_range
            ultrasonic_age = time.monotonic() - self._ultrasonic_stamp

        if ultrasonic_range is None or ultrasonic_age > self._ultrasonic_max_age:
            return depth, None
        ...
        scale = ultrasonic_range / ai_center_estimate
