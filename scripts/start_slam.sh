...
# setsid puts launch in a new process group so Ctrl+C (SIGINT to our group)
# doesn't reach it — giving us time to save the map first.
setsid ros2 launch rover_bringup slam.launch.py "$@" &
LAUNCH_PID=$!
